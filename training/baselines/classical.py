...
def structural_feature_vector(record: dict[str, Any]) -> np.ndarray:
    ...
    if descriptors.size:
        descriptor_stats = np.concatenate(
            [descriptors.mean(axis=0), descriptors.std(axis=0), descriptors.min(axis=0), descriptors.max(axis=0)]
        )
    else:
    ...
    simple_stats = np.asarray(
        [mean_degree, float(np.count_nonzero(composition)), float(np.max(composition)) if composition.size else 0.0],
        dtype=np.float64,
    )
...
def _metrics_payload(
    ...
    *,
    ...
    return calibrate_metrics(
        validation_logits, val_labels, test_logits,
        validation_score_prediction, val_score, val_mask,
        test_labels, test_score_prediction, test_score, test_mask,
    )


def _prediction_payload(logits: np.ndarray, score_prediction: np.ndarray) -> dict[str, np.ndarray]:
    return {"logits": np.asarray(logits, dtype=np.float64), "score_prediction": np.asarray(score_prediction, dtype=np.float64)}
...
def run_dummy(data: BenchmarkData, seed: int) -> dict[str, Any]:
    ...
    temperature, validation_metrics, test_metrics = _metrics_payload(
        data, validation_logits=val_logits, test_logits=test_logits,
        validation_score_prediction=val_score, test_score_prediction=test_score,
    )
    return {
        "parameter_count": 0, "training_seconds": time.time() - start, "temperature": temperature,
...
def run_xgboost(data: BenchmarkData, seed: int) -> dict[str, Any]:
    ...
    start = time.time()
    train_idx, val_idx, test_idx = data.splits["train"], data.splits["validation"], data.splits["test"]
    x_train, x_val, x_test = feature_matrix(data.records, train_idx), feature_matrix(data.records, val_idx), feature_matrix(data.records, test_idx)
    ...
    classifier = XGBClassifier(
        n_estimators=700, max_depth=6, learning_rate=0.035, subsample=0.90,
        colsample_bytree=0.90, min_child_weight=2.0, reg_alpha=1e-4, reg_lambda=1.0,
        objective="multi:softprob", num_class=3, eval_metric="mlogloss", random_state=int(seed),
        n_jobs=-1, tree_method="hist",
    )
    classifier.fit(x_train[valid_class], y_train[valid_class], sample_weight=base_weights[valid_class] * cw[y_train[valid_class]])
    regressor = XGBRegressor(
        n_estimators=700, max_depth=6, learning_rate=0.035, subsample=0.90,
        colsample_bytree=0.90, min_child_weight=2.0, reg_alpha=1e-4, reg_lambda=1.0,
        objective="reg:squarederror", eval_metric="mae", random_state=int(seed), n_jobs=-1, tree_method="hist",
    )
    fitted = bool(np.any(score_mask))
    if fitted:
        regressor.fit(x_train[score_mask], score_train[score_mask], sample_weight=base_weights[score_mask])
        val_score, test_score = regressor.predict(x_val), regressor.predict(x_test)
    else:
        val_score, test_score = np.full(len(x_val), 0.5), np.full(len(x_test), 0.5)
    ...
    val_logits, test_logits = np.log(np.clip(val_prob, 1e-8, 1.0)), np.log(np.clip(test_prob, 1e-8, 1.0))
    temperature, validation_metrics, test_metrics = _metrics_payload(
        data, validation_logits=val_logits, test_logits=test_logits,
        validation_score_prediction=val_score, test_score_prediction=test_score,
    )
